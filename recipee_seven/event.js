class event_list
{
        ...
        try {
                ...
                {
                buffer+=`
                <div class="search_inner image" onclick="id_setter('naad')"  >
        ...
        data_information()
        {
                ...
                {
                        $s().information.forEach(ev=>
                        {prn(ev)
                        prn(ev.summary.split(".Try<a")[0])
                        }
                        )
...
//                                 prn(ev)
//                   `              
//          <div class="dish"><div class="image">
//         //  <img src="https://spoonacular.com/recipeImages/$-556x370.jpg" 
//          class="image"
...
// border: solid #189ca2; set img border
        try {
                let id= window.location.hash.split('#').length
                id=window.location.hash.split('#')[id-1].split('=')[1]
                prn(id)


        } catch (error) {
                prn('window loc error',error)
        }

        try {
                let ret=__init__.EL.check_bulk_available(id)
                if(ret ==true)
                {
                        __init__.EL.data_creator_adder()
                ...
                {
                        prn('not available')
                        __init__.EL.pre_fetch(id,"information")
                }
        ...
        check_bulk_available(id)//$s().recipee_for_modal
        {
                try{
                        if($s().hasOwnProperty("information")==false)
                        {
                                return false
                        }
        ...
        // check_auto_id(id)
        // {
        //         // __init__.EL.check_bulk_available(id)
        //         try{
        //                 if($s().hasOwnProperty("autocomplete")== false)
        //                 {
        //                         return false
        ...
        //                 // Object.entries($s().autocomplete).forEach(ev=>{
        //                 //         ev=ev[1]
        ...
        id_setter(ev)
        {
                ...
                // prn('hey',arguments)
        //        prn(ev.path[1])
        document.location.hash+=`#id=${abc.id}` 
        prn(abc.id)
        $EE().mode_setter()     
        }
        callback_setup()
